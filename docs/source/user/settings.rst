...
.. code-block:: python
    ...
    {
        'default': 'http://{domain}/api/v1/{organization}/account/password/reset/confirm/?uid={uid}&token={token}'
    }
...
`organization_pk`: `my_frontend_domain/api/v1/{organization}/reset/path/?uid={uid}&token={token}`
